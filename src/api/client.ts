...
export async function request<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  const response = await fetch(buildUrl(path, query), {
    ...
    /*
      Ответы API устаревают мгновенно: задачи меняются в реальном
      времени, и показать вчерашнюю доску хуже, чем не показать ничего.

      no-store, а не значение по умолчанию: без явных заголовков от
      сервера браузер применяет эвристику из RFC 9111 и вправе счесть
      ответ свежим на своё усмотрение. Сервер такие заголовки теперь
      шлёт, но клиент не должен зависеть от того, обновлён ли бэкенд:
      у пользователя на руках может оказаться и старая сборка.
    */
    cache: 'no-store',
  })
