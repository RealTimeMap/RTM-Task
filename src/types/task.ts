 ...
 *
 * Намеренно отличается от серверного умолчания (там rtm-task, см.
 * DefaultProject в internal/domain/task/model.go): сервер отвечает за
 * задачи, у которых проект не указан вовсе — например, заведённые до
 * появления этого поля, — а форма подставляет тот проект, в который
 * задачи заводят чаще. Расхождения это не создаёт: форма всегда шлёт
 * project явно.
 */
